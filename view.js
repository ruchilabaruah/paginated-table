/**
 * Initialize the table data array, current page, records to show per page, sort states of the columns.
 * This acts like a store that keeps track of the global data.
 */
var countries = []; // Original records from backend
var currentDisplayedRecords = [];
var currentPage = 1;
var recordsPerPage = 5; // Configurable based on user input

/** Keep track of current sort applied on columns */
var sortStates = {
  sortCountryName: "",
  sortCountryCapital: "",
  sortCountryRegion: ""
...
/** Keep track of current filters applied on columns */
var filterStates = {
  countrySearchInput: "",
  capitalSearchInput: "",
  regionSearchInput: ""
...
var columns = [
  {
    ...
    sortId: "sortCountryName",
    filterId: "countrySearchInput",
    sortable: true,
    filterable: true
  },
  {
    display_name: "Capital",
    sortId: "sortCountryCapital",
    filterId: "capitalSearchInput",
    sortable: true,
    filterable: true
  },
  {
    display_name: "Region",
    sortId: "sortCountryRegion",
    filterId: "regionSearchInput",
    sortable: true,
    filterable: true
  }
];
...
/**
 * Rows to show per page is configurable. Adding an event listener for this.
 */
const rowsPerPage = document.getElementById("recordsPerPage");
rowsPerPage.value = recordsPerPage;
rowsPerPage.addEventListener("keyup", ev => {
  if (!isNaN(+ev.target.value)) {
    rowsPerPage.value = +ev.target.value;
    recordsPerPage = +ev.target.value;
    /**
     * This is an edge case where -- User is in say 3rd page having 5 records per page and with some filters applied, 
     * now he suddenly changes the records per page to 50, then the data should be updated with the filters still applied.
     * Page should also update accordingly.
     */
 ...
 */
const paginatedTable = document.getElementById("isPaginated");
paginatedTable.value = "yes";
paginatedTable.addEventListener("change", ev => {
  currentDisplayedRecords = [...countries];
  if (ev.target.value === "no") {
    recordsPerPage = countries.length;
  } else {
    recordsPerPage = rowsPerPage.value;
  }
  getDataAndUpdatePage(1);
});

/**
 * Asynchronous function to initially get data from the resource and construct the table with the entries.
 */
async function getCountries() {
  ...
  messenger.innerHTML = "<h3>Loading data, please wait</h3>";
  ...
  try {
    ...
    countries = await countriesPromise.json();
  } catch(err) {
    // Incase the api fails due to too many requests. (which is why I added here during development :-D)
    countries = [
      {
        name: "Afghanistan",
        topLevelDomain: [".af"],
  ...
  messenger.innerHTML = "<h3></h3>";
  // console.log("Countries", countries);

  /**
   * This variable wouldn't be required if we could make the api calls on page change, filters, sort.
   * Ideally api calls should be flexible enough to give us data on sorting, paginating. UI should
   * be only sending the current page number and count of records we need. Db data is closer to the
   * backend so all such retrieval of data should have its logic in the api calls (backend);
   */
  currentDisplayedRecords = [...countries];
...
/** Change filter state */
function changeFilterState(columnid) {
  const filterValue = document.getElementById(columnid).value;
  filterStates[columnid] = filterValue;
  getFilteredAndSortedData();
}

/**
 *
 * @param {*} columnid | selected column where the filter is applied
 * I am assuming here that on filtering, pagination will be reset to first page as the filter should
 * start searching the records from the first page onward
 */
function filterColumn(columnid, records) {
  const filterValue = filterStates[columnid];
  let filteredData = [];
  switch (columnid) {
    case "countrySearchInput":
      filteredData = records.filter(country =>
        country.name.toLowerCase().includes(filterValue)
      );
      break;
    case "capitalSearchInput":
      filteredData = records.filter(country =>
        country.capital.toLowerCase().includes(filterValue)
      );
      break;
    case "regionSearchInput":
      filteredData = records.filter(country =>
...
/** Change store/global state of column sort */
function changeSortState(columnid) {
  if (sortStates[columnid] === "asc") {
...
/**
 *
 * @param {*} columnid | Current column which the user is trying to sort
 */
function sortColumn(columnid, records) {
  const sortCol = document.getElementById(columnid);
  let colRows = [];
  let sortedRows = [];
  switch (columnid) {
    case "sortCountryName":
      colRows = records.map(country => country.name);
      colRows =
        sortStates[columnid] === "desc"
          ? colRows.sort().reverse()
          : colRows.sort();
      sortedRows = colRows.map(name => {
        const originalRow = records.find(coun => coun.name === name);
        return originalRow;
      });
      break;
    case "sortCountryCapital":
      colRows = records.map(country => country.capital);
      colRows =
        sortStates[columnid] === "desc"
          ? colRows.sort().reverse()
          : colRows.sort();
      sortedRows = colRows.map(capital => {
        const originalRow = records.find(coun => coun.capital === capital);
        return originalRow;
      });
      break;
    case "sortCountryRegion":
      colRows = records.map(country => country.region);
      colRows =
        sortStates[columnid] === "desc"
          ? colRows.sort().reverse()
          : colRows.sort();
      sortedRows = colRows.map(region => {
        const originalRow = records.find(coun => coun.region === region);
        return originalRow;
      });
      break;
    default:
...
function goToPrevPage() {
  if (currentPage > 1) {
    currentPage--;
    getDataAndUpdatePage(currentPage, currentDisplayedRecords);
  }
}
...
/**
 * Pagination handler | total number of pages that the table data
 * can be divided into based on recordsPerPage configured.
 */
function numberOfPages() {
  return Math.ceil(currentDisplayedRecords.length / recordsPerPage);
}
 ...
 */
function getDataAndUpdatePage(page, data) {
  currentPage = page;
  ...
  // Get the records from the original array and construct the table.
  if (!data) {
    data = [...countries];
  }

  /**
   * Slice data based on records to show per page
   */
  const recordsToShow = data.slice(
    (page - 1) * recordsPerPage,
    page * recordsPerPage
  );
  constructTableBody(recordsToShow);

  /**
   * Paginator: Page number buttons to show
   */
  const pageBtnCount = numberOfPages() - page > 5 ? 5 : numberOfPages() - page;
  let pageBtnsContainer = document.getElementById("pageButtons");
  pageBtnsContainer.innerHTML = "";
  for (let i = page; i < pageBtnCount + page; i++) {
    pageBtnsContainer.innerHTML += `<button type="button" onclick="getDataAndUpdatePage(${i}, currentDisplayedRecords)">${i}</button>`;
  }

  /**
   * Prev button will not be shown when viewing first page.
   * Next button will not be shown when viewing last page.
   */
  if (page == 1) {
...
/**
 * Construct the table header | columns you want to see, their click listeners and filter and sort configs.
 */
function constructTableHeader() {
  const tableData = document.getElementById("tableHeaders");
  let headers = ``;
  columns.forEach(col => {
    let colHeader = `<div style="display: flex; flex-direction: column">
      ${col.display_name}`;
    if (col.sortable) {
      colHeader += `<button type="button" id="${col.sortId}" onclick="changeSortState('${col.sortId}')"></button>`;
    }
    if (col.filterable) {
      colHeader += `<input
        type="text"
        id="${col.filterId}"
        placeholder="Search"
        onkeyup="changeFilterState('${col.filterId}')"
      >
    </div>`;
    }
    headers += colHeader;
  });
  const tableHead = `<div class="tableHeaders">${headers}</div>`;
  tableData.innerHTML = tableHead;
}

/**
 * Whenever data is to be updated in the table, we need to consider the filters and sort applied.
 * @param {*} data
 */
function getFilteredAndSortedData() {
  let updatedRecords = [...countries];
  // check if filters are applied on any column
  Object.keys(filterStates).forEach(filterKey => {
    if (filterStates[filterKey]) {
      const filteredList = filterColumn(filterKey, updatedRecords);
      updatedRecords = [...filteredList];
    }
  });
  console.log("After filtering: Records =", updatedRecords);

  // check if sorting is applied on any column
  Object.keys(sortStates).forEach(sortKey => {
    if (sortStates[sortKey]) {
      const sortedList = sortColumn(sortKey, updatedRecords);
      updatedRecords = [...sortedList];
    }
  });
  console.log("After sorting: Records =", updatedRecords);

  // Save this filtered and sorted data in an array so that when we change page number, pagination is applied on this data and not on whole data received from backend
  currentDisplayedRecords = [...updatedRecords];
  if (paginatedTable.value === "yes") {
    getDataAndUpdatePage(1, updatedRecords);
  } else {
    constructTableBody(updatedRecords);
  }
}

/**
 * Get table id and add the rows
 */
function constructTableBody(data) {
  const tableData = document.getElementById("tableBody");
  tableData.innerHTML = "";
  data.forEach(country => {
    tableData.innerHTML += `<div class="tableRow"><div>${country.name}</div><div>${country.capital}</div><div>${country.region}</div></div>`;
  });
}
