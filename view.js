...
function filterColumn(columnid) {
  ...
  getDataAndUpdatePage(1, filteredData);
}
...
function sortColumn(columnid) {
  ...
  switch (columnid) {
    case "sortCountryName":
      colRows = currentDisplayedRecords.map(country => country.name);
      colRows = sortStates[columnid] === "desc" ? colRows.sort().reverse() : colRows.sort();
      sortedRows = colRows.map(name => {
        const originalRow = currentDisplayedRecords.find(
          coun => coun.name === name
        );
        return originalRow;
      });
    ...
    case "sortCountryCapital":
      colRows = currentDisplayedRecords.map(country => country.capital);
      colRows = sortStates[columnid] === "desc" ? colRows.sort().reverse() : colRows.sort();
      sortedRows = colRows.map(capital => {
        const originalRow = currentDisplayedRecords.find(
          coun => coun.capital === capital
        );
        return originalRow;
      });
      break;
    case "sortCountryRegion":
      colRows = currentDisplayedRecords.map(country => country.region);
      colRows = sortStates[columnid] === "desc" ? colRows.sort().reverse() : colRows.sort();
      sortedRows = colRows.map(region => {
        const originalRow = currentDisplayedRecords.find(
          coun => coun.region === region
        );
        return originalRow;
      });
      break;
    default:
      sortedRows = [...currentDisplayedRecords];
  }
  debugger;
  sortCol.innerHTML =
    sortStates[columnid] === "desc"
...
function goToPrevPage() {
  if (currentPage > 1) {
    currentPage--;
    getDataAndUpdatePage(currentPage);
  }
}
...
function getDataAndUpdatePage(page, data) {
  ...
  currentDisplayedRecords = [...recordsToShow];
  ...
  for (let i = page; i < pageBtnCount + page; i++) {
    pageBtnsContainer.innerHTML += `<button type="button" onclick="getDataAndUpdatePage(${i})">${i}</button>`;
  }
...
function constructTableHeader() {
  ...
  const tableHead = `<div class="tableHeaders">
  <div style="display: flex; flex-direction: column">
    Country
    <button type="button" id="sortCountryName" onclick="sortColumn('sortCountryName')"></button>
    <input
      type="text"
      id="countrySearchInput"
      placeholder="Search"
      onkeyup="filterColumn('countrySearchInput')"
    >
  </div>
  <div style="display: flex; flex-direction: column">
    Capital
    <button type="button" id="sortCountryCapital" onclick="sortColumn('sortCountryCapital')">Sort</button>
    <input
      type="text"
      id="capitalSearchInput"
      placeholder="Search"
      onkeyup="filterColumn('capitalSearchInput')"
    >
  </div>
  <div style="display: flex; flex-direction: column">
    Region
    <button type="button" id="sortCountryRegion" onclick="sortColumn('sortCountryRegion')">Sort</button>
    <input
      type="text"
      id="regionSearchInput"
      placeholder="Search"
      onkeyup="filterColumn('regionSearchInput')"
    >
  </div>
</div>`;
